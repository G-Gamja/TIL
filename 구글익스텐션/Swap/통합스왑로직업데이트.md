익스텐션에서 추가될 통합스왑의 기능은 다음과 같은 프로세스를 거친다
![통합스왑](/images/IS_update_version.jpg)

위 프로세스는 아래의 가정하에 작성되었다
- 지원할 체인은 기존 익스텐션 및 코스모스테이션에서 지원하기로 결정한 체인으로 한정한다.
- Squid의 지원 범위는 EVM to EVM, EVM -> Cosmos(일부), 타 체인간 Swap만 지원하는 버젼이다.
- 송,수신 체인을 선택하기 전까지는 사용할 Swap api가 정해지지 않기 때문에 토큰 선택을 block한다.

# 체인 선택 프로세스

Squid SDK가 업데이트되면서 Cosmos계열의 체인을 수신체인에서 선택할 수 있게 되었다.

따라서 송,수신 선택에 따라 맞은편 체인 선택에 영향이 가기 때문에 송, 수신 체인의 선택 순서를 분류할 필요가 있다.

## 송신 가능 체인 리스팅
```ts
const fromChainList = [...squid.chains.filter((item)=>item.type ==='evm'),...oneInch.chains,Osmosis].unique();
```
## 수신 가능 체인 리스팅
`isCosmosCover`필드는 해당 코스모스 체인이 수신 체인으로 선택가능한지 여부를 나타내는 필드이다.

- 해당 필드는 작성자가 선언한 새 속성임.
```ts
const toChainList = [...squid.chains.filter((item)=>item.type ==='evm'|| item.isCosmosCover === 'true'),...oneInch.chains].unique();
```
## 송신 체인을 먼저 선택하는 경우

1. 송신 체인을 선택한다.
    - 이때 선택 가능한 체인은 Squid가 지원하는 체인(EVM계열만) + 1inch가 지원하는 체인 + Osmosis이다.
        - Squid와 1inch를 중복 지원하는 체인의 경우는 Squid에서 제공하는 지원 체인 데이터를 우선시 한다. 즉 1inch에서 squid와 중복되는 체인은 squid의 체인으로 override한다.(각 API에서 뿌려주는 리스트의 아이템의 타입이 다르긴 하지만 중요한 필드로는 chain name, chain id, img등만 있으면 될 것으로 예상되어 세 API의 리스트 타입을 합한 유니언 타입으로 선언해주면 될 것으로 예상함)
    - 각 체인 마다 지원하는 api를 나타내는 추가 필드를 선언하여 알맞은 값을 할당해준다. ex) `const a = Chains[0].supportedApi //a === 'SQUID'`
2. 선택된 체인을 바탕으로 수신 체인의 선택 가능한 체인 리스트를 필터링 한다.
    - 1. 만약 선택된 체인이 EVM계열 이라면
        - 선택된 체인의 api지원 여부를 체크
            1. Squid, 1inch 모두 지원
                - 어떤 체인을 선택했건 필터링없이 수신 체인 리스트 사용가능
                1. 첫 선택과 동일한 체인을 선택시
                    - `1inch swap api 선택 확정`
                2. 첫 선택과 다른 체인을 선택시
                     - `Squid SDK 선택 확정`
            2. 1inch만 지원
                - 1단계에서 선택한 체인과 동일한 체인만 선택가능
                - 따라서 이때 선택 가능한 체인 리스트에는 선택했던 체인만 들어있음
                - `수신 체인 리스트 = [첫 선택 체인]`
                - `1inch swap api 선택 확정`
            3. Squid만 지원
                - 이런 케이스는 코스모스테이션에서 지원하지 않는다.(ex. Celo chain)
    - 2. 만약 선택된 체인이 Cosmos계열이라면
        - 현재는 Squid에서 송신체인에서 코스모스 계열을 지원하지 않기 때문에 Osmosis체인만 선택 가능하므로 이 단계에서는 Osmosis를 선택했다고 확신 가정한다.
        - 따라서 맞은 편 체인 리스트에는 Osmosis만 포함되어 있음
        - `Osmosis Swap 선택 확정`
## 수신 체인을 먼저 선택하는 경우

1. 수신 체인을 선택한다.
    - 이때 선택 가능한 체인은 Squid가 지원하는 체인(EVM계열, Cosmos계열 일부) + 1inch가 지원하는 체인이다.

2. 선택된 체인을 바탕으로 맞은 편 체인의 선택 가능한 체인 리스트를 필터링 한다.
    - 1. 만약 선택된 체인이 EVM계열 이라면
        - 선택된 체인의 api지원 여부를 체크
            1. Squid, 1inch 모두 지원
                - 다음의 리스트에서 선택가능
                    - `체인 리스트 = [...squid.filter((item)=item.type==='evm')]`
                    - 스퀴드에서 지원하는 체인중 EVM계열만을 의미함
                1. 첫 선택과 동일한 체인을 선택시
                    - `1inch swap api 선택 확정`
                2. 첫 선택과 다른 체인을 선택시
                  - `Squid SDK 선택 확정`
            2. 1inch만 지원
                - 1단계에서 선택한 체인과 동일한 체인만 선택가능
                - 따라서 이때 선택 가능한 체인 리스트에는 선택했던 체인만 들어있음
                - `수신 체인 리스트 = [첫 선택 체인]`
                1. `1inch swap api 선택 확정`
            3. Squid만 지원
                - 이런 케이스는 코스모스테이션에서 지원하지 않는다.(ex. Celo chain)
    - 2. 만약 선택된 체인이 Cosmos계열이라면
        - 현재 수신 체인에서 선택가능한 Cosmos 계열의 체인은 다음과 같다
            - Squid에서 지원하는 일부 Cosmos 계열 체인
        - 선택한 체인이 Osmosis체인인지 체크
            - Yes
                - 다음의 리스트 내에서 송신 체인 선택가능
                - `[...squid.filter((item)=item.type==='evm'),Osmosis]` 
                1. 송신 체인으로 Osmosis를 선택한 경우
                    - `Osmosis Swap 선택 확정`
                2. 송신 체인으로 Osmosis외 체인을 선택한 경우
                    - `Squid SDK 선택 확정`
            - No
                - 다음의 리스트 내에서 선택 가능
                - `[...squid.filter((item)=item.type==='evm')]`
                - 어떤 체인을 선택하든 Squid api를 사용해야함
                - `Squid SDK 선택 확정`

# 토큰 선택 프로세스

![통합스왑](/images/IntegratedSwap_Token.jpg)


1. Squid Swap 
    1. 선택한 두 체인이 EVM이며 다른 체인일때
    - 선택한 각 체인이 지원하는 토큰의 리스트 내에서 선택 가능함
    
    ```ts
    import { Squid } from "@0xsquid/sdk";

        (async () => {
        // instantiate the SDK
        const squid = new Squid({
            baseUrl: "https://testnet.api.0xsquid.com" // for mainnet use "https://api.0xsquid.com"
        });
        // init the SDK
        await squid.init();
        console.log("Squid inited");
        })();

        // filtering tokenList by selecetd chain id
        const selectedFromChainId = fromChain.chainID
        const tokenList = squid.tokens.find(
        t =>
        t.chainId === selectedFromChainId
    );
      ```
    - 이때 tokenList는 선택된 송신 체인에서 선택가능한 토큰 리스트를 의미
    - response
        ```json
            {
                "status": true,
                "data": {
                    "tokens": [
                    {
                        "chainId": 1284,
                        "address": "0xAcc15dC74880C9944775448304B263D191c6077F",
                        "name": "Wrapped GLMR",
                        "symbol": "WGLMR",
                        "decimals": 18,
                        "logoURI": "https://assets.coingecko.com/coins/images/22459/small/glmr.png?1641880985",
                        "coingeckoId": "wrapped-moonbeam"
                    },
                    ...
                    ]
                }
        }
    2. 송신 체인: EVM , 수신 체인: Cosmos
        - 송신 체인: 선택한 체인이 지원하는 모든 토큰 선택가능
        - 수신 체인: axlUSDC + 송신 토큰과 `match`되는 선택한 수신 체인에서 지원하는 토큰
            - 코스모스가 수신 체인일 경우 현재는 axlUSDC가 기본으로 수신이 가능하며, 추가로 송신 토큰과 match되는 토큰이 수신 체인에서 지원하고 있을 때 선택이 가능해진다. ex) Dai <=> dai_weth
            - Match 여부는 `Axelar`에서 제공하는 json파일 [Axelar Supported Tokens](/%EB%B8%94%EB%A1%9D%EC%B2%B4%EC%9D%B8/research/axelar/assets/evm_assets.json) 통해 필터링이 가능하다
                - 선택한 송신 토큰의 Symbol값을 얻는다 -> json안에서 객체의 심볼이 송신 토큰의 Symbol값과 일치하는 객체를 얻는다 -> 해당 객체의 `id`값을 얻는다 -> 스퀴드 체인에서 선택한 코스모스 체인으로 필터링 한 토큰리스트에서 `id`값이 이전 단계에서 얻은 값과 일치하는 지 체크, 일치한다면 `match`되는 토큰으로 판단    

                ! 단 필터링 로직에서 `symbol`을 사용하는 것은 위험할 수 있음(중복값 위험)
    3. 송신이 Cosmos인 경우
        - 아직 지원하지 않고 있음. 23년도 1분기에 지원될 예정

2. 1inch Swap (선택한 두 체인이 EVM이며 같은 체인일때)
    - 선택한 체인이 지원하는 토큰의 리스트 내에서 선택 가능함
        - 단 송,수신 상관없이 먼저 선택한 토큰은 추후 토큰 선택시 제외된다.
    - `https://api.1inch.io/v5.0/`${evmChainId=1}`/tokens`
    - response
        - ````json
            {
                "tokens": {
                    "0x0327112423f3a68efdf1fcf402f6c5cb9f7c33fd": {
                    "symbol": "BTC++",
                    "name": "PieDAO BTC++",
                    "decimals": 18,
                    "address": "0x0327112423f3a68efdf1fcf402f6c5cb9f7c33fd",
                    "logoURI": "https://tokens.1inch.io/0x0327112423f3a68efdf1fcf402f6c5cb9f7c33fd.png",
                    "tags": [
                        "tokens",
                        "PEG:BTC"
                    ]
                    ...
                    }
            },
        ```
3. Osmosis Swap
    - 기존 구현안 대로 chain list를 토대로 풀이 정의되어있는 토큰만 사용


# Trasfer Type 미지원되는 부분 - Squid

- EVM <-> Cosmos일때 코스모스 계열에서 받는 토큰이 axlUSDC이 아닌 경우 다음과 같은 오류가 발생
- 양 체인이 서로 호환(match)되는 토큰이 아닌 경우 에러 발생
    - ex) dai-weth <-> 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE 인 경우는 안되지만
    - dai-weth <-> 0x6B175474E89094C44Da98b954EedeAC495271d0F(Dai address)는 가능

 - Squid에서 support된다고 나온 토큰들 중 기존 Axelar SDK에서 지원하는 브릿지 기능을 사용할 수 있는(송 수신 토큰이 matching 되는 e.g. dai - dai-wei(axelar dai))토큰들은 정상 작동이 되지만 이 외 토큰들은 아래와 같은 오류를 출력하고 있음
- 추가 개발(단순 브릿지 + 스왑으로 추정)이 진행중이기에 Squid 디앱에서 막아놓은 것으로 추정함

```json
{
  "errors": [
    {
      "errorType": "NotSupportedError",
      "message": "To get a token other than axlUSDC on a Cosmos chain, please set the from and to tokens to match, e.g. Ethereum ETH and Cosmos axlWETH. Check our docs for a list of supported tokens."
    }
  ]
}
```

- 코스모스 계열 체인에서 선택한 토큰이 axlUSDC라면 
    - EVM계열에서는 어떤 토큰을 설정해도 상관없음
- 코스모스 계열 체인에서 선택한 토큰이 axlUSDC이 아니라면 
    - EVM계열에서는 코스모스 계열 체인에서 선택한 토큰과 match되는 토큰만 선택가능
    - ex. dai-weth <-> 0x6B175474E89094C44Da98b954EedeAC495271d0F(dai address)

```json
    {
  "route": {
    "estimate": {
      "fromAmount": "100000000000000000000",
      "sendAmount": "100000000000000000000",
      "toAmount": "99999999999989500000",
      "toAmountMin": "99999999999989500000",
      "fromAmountUSD": "100,000,000,000,000.00",
      "route": {
        "fromChain": [
          {
            "type": "BRIDGE",
            "fromToken": {
              "name": "Tether USD",
              "address": "0xdAC17F958D2ee523a2206206994597C13D831ec7",
              "symbol": "USDT",
              "decimals": 6,
              "chainId": 1,
              "logoURI": "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xdAC17F958D2ee523a2206206994597C13D831ec7/logo.png",
              "coingeckoId": "tether",
              "commonKey": "uusdt"
            },
            "toToken": {
              "chainId": "cosmoshub-4",
              "address": "uusdt",
              "name": "Axelar USDT",
              "symbol": "axlUSDT",
              "decimals": 6,
              "logoURI": "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xdAC17F958D2ee523a2206206994597C13D831ec7/logo.png",
              "coingeckoId": "tether",
              "commonKey": "uusdt",
              "ibcDenom": "ibc/5662412372381F56C5F83A0404DC7209E5143ABD32EF67B5705DBE8D9C2BF001"
            },
            "fromAmount": "100000000000000000000",
            "toAmount": "99999999999989500000",
            "toAmountMin": "99999999999989500000",
            "exchangeRate": "0.999999999999895",
            "priceImpact": "0"
          }
        ],
        "toChain": []
      },
      "feeCosts": [
        {
          "name": "Axelar Fee",
          "description": "Axelar bridge fee",
          "percentage": "0",
          "token": {
            "name": "Tether USD",
            "address": "0xdAC17F958D2ee523a2206206994597C13D831ec7",
            "symbol": "USDT",
            "decimals": 6,
            "chainId": 1,
            "logoURI": "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xdAC17F958D2ee523a2206206994597C13D831ec7/logo.png",
            "coingeckoId": "tether",
            "commonKey": "uusdt"
          },
          "amount": "10500000",
          "amountUSD": "10.5000"
        }
      ],
      "gasCosts": [
        {
          "type": "executeCall",
          "token": {
            "chainId": 1,
            "address": "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
            "name": "Ethereum",
            "symbol": "ETH",
            "decimals": 18,
            "logoURI": "https://tokens.1inch.io/0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee.png",
            "coingeckoId": "ethereum",
            "commonKey": "weth-wei"
          },
          "amount": "4089569571780000",
          "amountUSD": "6.9832",
          "gasPrice": "27711211227",
          "maxFeePerGas": "56674661546",
          "maxPriorityFeePerGas": "1500000000",
          "estimate": "140000",
          "limit": "140000"
        }
      ],
      "estimatedRouteDuration": 960,
      "aggregatePriceImpact": "0",
      "exchangeRate": "0.999999999999895",
      "toAmountUSD": "99,999,999,999,989.50"
    },
    "params": {
      "slippage": 1,
      "toAddress": "cosmos1aygdt8742gamxv8ca99wzh56ry4xw5s39smmhm",
      "fromAmount": "100000000000000000000",
      "toToken": {
        "chainId": "cosmoshub-4",
        "address": "uusdt",
        "name": "Axelar USDT",
        "symbol": "axlUSDT",
        "decimals": 6,
        "logoURI": "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xdAC17F958D2ee523a2206206994597C13D831ec7/logo.png",
        "coingeckoId": "tether",
        "commonKey": "uusdt",
        "ibcDenom": "ibc/5662412372381F56C5F83A0404DC7209E5143ABD32EF67B5705DBE8D9C2BF001"
      },
      "fromToken": {
        "name": "Tether USD",
        "address": "0xdAC17F958D2ee523a2206206994597C13D831ec7",
        "symbol": "USDT",
        "decimals": 6,
        "chainId": 1,
        "logoURI": "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xdAC17F958D2ee523a2206206994597C13D831ec7/logo.png",
        "coingeckoId": "tether",
        "commonKey": "uusdt"
      },
      "toChain": "cosmoshub-4",
      "fromChain": "1"
    },
    "transactionRequest": {
      "routeType": "BRIDGE",
      "targetAddress": "0x4F4495243837681061C4743b74B3eEdf548D56A5",
      "data": "",
      "value": "0",
      "gasLimit": "140000",
      "gasPrice": "27711211227",
      "maxFeePerGas": "56674661546",
      "maxPriorityFeePerGas": "1500000000"
    }
  }
}
```